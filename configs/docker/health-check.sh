...
if ! grep -i -E "ga" .liferay-version; then
	_log "Checking for license registration..."
	if ! grep -i -E "license validation passed" logs/liferay.*.log; then
...
if ! curl -s localhost:8080 -o /dev/null --write-out "%{http_code}"; then
